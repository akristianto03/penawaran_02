...
class Msheet_pile extends CI_Model {

    function get_latest() 
    {
        $query = $this->db->query("SELECT * FROM latest_update WHERE produk='Sheet Pile' ORDER BY tanggal DESC");
        return $query->result();
    }


    function get_row_data_sheet_pile($id) 
    {
        return $this->db->where('id', $id)->get('tbl_data_penawaran')->row_array();
    }
    
    // MODEL QUERY USER sheet_pile

    public function get_data_sheet_pile($username)
    {
        $query = $this->db->query("SELECT * FROM tbl_data_penawaran WHERE marketing='$username' and produk='Sheet Pile' ORDER BY id DESC");
        return $query->result();
    }

    public function get_detail_sheet_pile($id){
    ...
    {
        $query = $this->db->query("SELECT * FROM tbl_permintaan_sheet_pile sheet_pile JOIN tbl_data_penawaran penawaran on sheet_pile.nama_proyek=penawaran.nama_proyek WHERE penawaran.marketing='$username' and penawaran.id='$id' and penawaran.produk='Sheet Pile' ");
        return $query->result();
    }
    ...
    public function add_permintaan_sheet_pile()
    {
        ...
        $save = $this->db
                     ->insert('tbl_permintaan_sheet_pile', $data);	
    ...
    {
        $hapus = $this->db->where('ids',$ids)->delete('tbl_permintaan_sheet_pile');
        if($hapus)
        {
            return TRUE;
        }else
    ...
    public function get_data_sheet_pile_admin()
    {
        return $this->db->where('produk','Sheet Pile')->order_by('id','DESC')->get('tbl_data_penawaran')->result();
    }

    public function get_detail_sheet_pile_admin($id){
        $query = $this->db->query("SELECT * from tbl_data_penawaran where id='$id'");
        $result = $query->result_array();
        return $result;
    }
    ...
    {
        $query = $this->db->query("SELECT penawaran.nama_proyek, penawaran.id, sheet_pile.ids, sheet_pile.nama_proyek, sheet_pile.marketing, sheet_pile.item, sheet_pile.panjang, sheet_pile.keterangan ,sheet_pile.harga_satuan, sheet_pile.volume, penawaran.ppn, sheet_pile.jumlah FROM tbl_permintaan_sheet_pile sheet_pile JOIN tbl_data_penawaran penawaran on penawaran.nama_proyek=sheet_pile.nama_proyek WHERE penawaran.id='$id' ");
        return $query->result();
    }

    public function update_permintaan_sheet_pile($ids)
    {
        $dt = new DateTime("now", new DateTimeZone('Asia/Jakarta'));
        $data = array(
            'marketing'     => $this->input->post('marketing'),
        ...
        $update = $this->db->where('ids', $ids)->update('tbl_permintaan_sheet_pile', $data);	
    ...
    public function pdf_data_all_sheet_pile($id)
    {
        $query = $this->db->query("SELECT * FROM tbl_data_penawaran where id='$id' and produk='Sheet Pile' ");
        return $query->result_array();
    }

    public function pdf_data_permintaan_sheet_pile($id)
    {
        $query = $this->db->query("SELECT penawaran.nama_proyek, penawaran.id, sheet_pile.ids, sheet_pile.nama_proyek, sheet_pile.marketing, sheet_pile.item, sheet_pile.panjang, sheet_pile.keterangan , sheet_pile.volume ,sheet_pile.harga_satuan , sheet_pile.jumlah, penawaran.include ,penawaran.ppn ,(sheet_pile.volume * sheet_pile.harga_satuan) as total_perkalian  FROM tbl_permintaan_sheet_pile sheet_pile JOIN tbl_data_penawaran penawaran on penawaran.nama_proyek=sheet_pile.nama_proyek WHERE penawaran.id='$id' and penawaran.produk='Sheet Pile'");
        return $query->result();
    }

    public function pdf_hitung_ppn($id)
    {
        $query = $this->db->query("SELECT penawaran.nama_proyek, penawaran.id, sheet_pile.ids, sheet_pile.nama_proyek, sheet_pile.harga_satuan , sheet_pile.volume, sheet_pile.keterangan ,sheet_pile.jumlah, penawaran.ppn ,sum((sheet_pile.volume * sheet_pile.harga_satuan)*0.11) as total_ppn  FROM tbl_permintaan_sheet_pile sheet_pile JOIN tbl_data_penawaran penawaran on penawaran.nama_proyek=sheet_pile.nama_proyek WHERE penawaran.id='$id'  and penawaran.produk='Sheet Pile'");
        return $query->result_array();
    }
    ...
    {
        $query = $this->db->query("SELECT penawaran.nama_proyek, penawaran.id, sheet_pile.volume, sheet_pile.ids, sheet_pile.nama_proyek,sheet_pile.harga_satuan, sheet_pile.keterangan , sheet_pile.jumlah, penawaran.ppn ,sum(sheet_pile.volume * sheet_pile.harga_satuan) as total_volume  FROM tbl_permintaan_sheet_pile sheet_pile JOIN tbl_data_penawaran penawaran on penawaran.nama_proyek=sheet_pile.nama_proyek WHERE penawaran.id='$id' and penawaran.produk='Sheet Pile'");
        return $query->result_array();
    }

    public function pdf_total_jumlah_total_ppn($id)
    {
        $query = $this->db->query("SELECT penawaran.nama_proyek, penawaran.id, sheet_pile.ids, sheet_pile.nama_proyek, sheet_pile.harga_satuan , sheet_pile.jumlah, sheet_pile.keterangan, sheet_pile.volume ,penawaran.ppn ,sum(sheet_pile.volume * sheet_pile.harga_satuan) as total_volume ,sum((sheet_pile.volume * sheet_pile.harga_satuan)*0.11) as total_ppn FROM tbl_permintaan_sheet_pile sheet_pile JOIN tbl_data_penawaran penawaran on penawaran.nama_proyek=sheet_pile.nama_proyek WHERE penawaran.id='$id' and penawaran.produk='Sheet Pile'");
        return $query->result_array();
    }

    public function pdf_get_bulan_sheet_pile($id)
    {
        $query = $this->db->query("SELECT MONTH(tanggal) as tanggal from tbl_data_penawaran where id='$id' and produk='Sheet Pile'");
        return $query->result_array();
    }

    public function pdf_get_tahun_sheet_pile($id)
    {
        $query = $this->db->query("SELECT YEAR(tanggal) as tanggal from tbl_data_penawaran where id='$id' and produk='Sheet Pile'");
        return $query->result_array();
    }

    public function pdf_get_tanggal_sheet_pile($id)
    {
        $query = $this->db->query("SELECT DAY(tanggal) as tanggal from tbl_data_penawaran where id='$id' and produk='Sheet Pile'");
        return $query->result_array();
    }
}