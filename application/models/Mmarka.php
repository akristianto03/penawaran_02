...
class Mmarka extends CI_Model {

    function get_latest() 
    {
        $query = $this->db->query("SELECT * FROM latest_update WHERE produk='Marka Jalan' ORDER BY tanggal DESC");
        return $query->result();
        // return $this->db->get('latest_update')->result();
    }

    function get_row_data_marka($id) 
    {
        return $this->db->where('id', $id)->get('tbl_data_penawaran')->row_array();
    }
    
    // MODEL QUERY USER marka 

    public function get_data_marka($username)
    {
        $query = $this->db->query("SELECT * FROM tbl_data_penawaran WHERE marketing='$username' and produk='Marka Jalan' ORDER BY id DESC");
        return $query->result();
    }

    public function get_detail_marka($id){
    ...
    {
        $query = $this->db->query("SELECT * FROM tbl_permintaan_marka marka JOIN tbl_data_penawaran penawaran on marka.nama_proyek=penawaran.nama_proyek WHERE penawaran.marketing='$username' and penawaran.id='$id' and penawaran.produk='Marka Jalan' ");
        return $query->result();
    }
    ...
    public function add_permintaan_marka()
    {
        ...
        $save = $this->db
                     ->insert('tbl_permintaan_marka', $data);	
    ...
    {
        $hapus = $this->db->where('ids',$ids)->delete('tbl_permintaan_marka');
        if($hapus)
        {
            return TRUE;
        }else
    ...
    public function get_data_marka_admin()
    {
        return $this->db->where('produk','Marka Jalan')->order_by('id','DESC')->get('tbl_data_penawaran')->result();
    }

    public function get_detail_marka_admin($id){
        $query = $this->db->query("SELECT * from tbl_data_penawaran where id='$id'");
        $result = $query->result_array();
        return $result;
    }
    ...
    {
        $query = $this->db->query("SELECT penawaran.nama_proyek, penawaran.id, marka.ids, marka.nama_proyek, marka.marketing, marka.item, marka.satuan ,marka.harga_satuan, penawaran.include, penawaran.ppn, marka.jumlah, marka.standard FROM tbl_permintaan_marka marka JOIN tbl_data_penawaran penawaran on penawaran.nama_proyek=marka.nama_proyek WHERE penawaran.id='$id' ");
        return $query->result();
    }

    public function update_permintaan_marka($ids)
    {
        $dt = new DateTime("now", new DateTimeZone('Asia/Jakarta'));
        $data = array(
            'marketing'     => $this->input->post('marketing'),
        ...
        $update = $this->db->where('ids', $ids)->update('tbl_permintaan_marka', $data);	
    ...
    public function pdf_data_all_marka($id)
    {
        $query = $this->db->query("SELECT * FROM tbl_data_penawaran where id='$id' and produk='Marka Jalan' ");
        return $query->result_array();
    }

    public function pdf_data_permintaan_marka($id)
    {
        $query = $this->db->query("SELECT penawaran.nama_proyek, penawaran.id, marka.ids, marka.nama_proyek, marka.marketing, marka.item, marka.satuan ,marka.harga_satuan , marka.jumlah, marka.standard, penawaran.include ,penawaran.ppn ,(marka.jumlah * marka.harga_satuan) as total_perkalian  FROM tbl_permintaan_marka marka JOIN tbl_data_penawaran penawaran on penawaran.nama_proyek=marka.nama_proyek WHERE penawaran.id='$id' and penawaran.produk='Marka Jalan'");
        return $query->result();
    }

    public function pdf_hitung_ppn($id)
    {
        $query = $this->db->query("SELECT penawaran.nama_proyek, penawaran.id, marka.ids, marka.standard, marka.nama_proyek, marka.harga_satuan , marka.jumlah, penawaran.ppn ,sum((marka.jumlah * marka.harga_satuan)*0.11) as total_ppn  FROM tbl_permintaan_marka marka JOIN tbl_data_penawaran penawaran on penawaran.nama_proyek=marka.nama_proyek WHERE penawaran.id='$id' and penawaran.produk='Marka Jalan'");
        return $query->result_array();
    }
    ...
    {
        $query = $this->db->query("SELECT penawaran.nama_proyek, penawaran.id, marka.ids, marka.standard, marka.nama_proyek,marka.harga_satuan , marka.jumlah, penawaran.ppn ,sum(marka.jumlah * marka.harga_satuan) as total_jumlah  FROM tbl_permintaan_marka marka JOIN tbl_data_penawaran penawaran on penawaran.nama_proyek=marka.nama_proyek WHERE penawaran.id='$id' and penawaran.produk='Marka Jalan'");
        return $query->result_array();
    }

    public function pdf_total_jumlah_total_ppn($id)
    {
        $query = $this->db->query("SELECT penawaran.nama_proyek, penawaran.id, marka.ids, marka.standard, marka.nama_proyek, marka.harga_satuan , marka.jumlah ,penawaran.ppn ,sum(marka.jumlah * marka.harga_satuan) as total_jumlah ,sum((marka.jumlah * marka.harga_satuan)*0.11) as total_ppn FROM tbl_permintaan_marka marka JOIN tbl_data_penawaran penawaran on penawaran.nama_proyek=marka.nama_proyek WHERE penawaran.id='$id' and penawaran.produk='Marka Jalan'");
        return $query->result_array();
    }

    public function pdf_get_bulan_marka($id)
    {
        $query = $this->db->query("SELECT MONTH(tanggal) as tanggal from tbl_data_penawaran where id='$id' and produk='Marka Jalan'");
        return $query->result_array();
    }

    public function pdf_get_tahun_marka($id)
    {
        $query = $this->db->query("SELECT YEAR(tanggal) as tanggal from tbl_data_penawaran where id='$id' and produk='Marka Jalan'");
        return $query->result_array();
    }

    public function pdf_get_tanggal_marka($id)
    {
        $query = $this->db->query("SELECT DAY(tanggal) as tanggal from tbl_data_penawaran where id='$id' and produk='Marka Jalan' ");
        return $query->result_array();
    }
}