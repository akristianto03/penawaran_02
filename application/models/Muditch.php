...
class Muditch extends CI_Model {

    function get_latest() 
    {
        $query = $this->db->query("SELECT * FROM latest_update WHERE produk='Uditch' ORDER BY tanggal DESC");
        return $query->result();
    }

    function get_row_data_uditch($id) 
    {
        return $this->db->where('id', $id)->get('tbl_data_penawaran')->row_array();
    }
    
    // MODEL QUERY USER UDITCH

    public function get_data_uditch($username)
    {
        $query = $this->db->query("SELECT * FROM tbl_data_penawaran WHERE marketing='$username' and produk='Uditch' ORDER BY id DESC");
        return $query->result();
    }

    public function get_detail_uditch($id){
    ...
    {
        $query = $this->db->query("SELECT * FROM tbl_permintaan_uditch uditch JOIN tbl_data_penawaran penawaran on uditch.nama_proyek=penawaran.nama_proyek WHERE penawaran.marketing='$username' and penawaran.id='$id' and penawaran.produk='Uditch' ");
        return $query->result();
    }
    ...
    public function add_permintaan_uditch()
    {
        ...
        $data = array(
            ...
            'beban_gandar'     => $this->input->post('beban_gandar'),
        ...
        $save = $this->db
                     ->insert('tbl_permintaan_uditch', $data);	
    ...
    {
        $hapus = $this->db->where('ids',$ids)->delete('tbl_permintaan_uditch');
        if($hapus)
        {
            return TRUE;
        }else
    ...
    public function get_data_uditch_admin()
    {
        return $this->db->where('produk','Uditch')->order_by('id','DESC')->get('tbl_data_penawaran')->result();
    }

    public function get_detail_uditch_admin($id){
        $query = $this->db->query("SELECT * from tbl_data_penawaran where id='$id'");
        $result = $query->result_array();
        return $result;
    }
    ...
    {
        $query = $this->db->query("SELECT penawaran.nama_proyek, penawaran.id, uditch.ids, uditch.nama_proyek, uditch.marketing, uditch.item, uditch.beban_gandar ,uditch.harga_satuan, penawaran.include , uditch.keterangan, penawaran.ppn, uditch.jumlah FROM tbl_permintaan_uditch uditch JOIN tbl_data_penawaran penawaran on penawaran.nama_proyek=uditch.nama_proyek WHERE penawaran.id='$id' ");
        return $query->result();
    }

    public function update_permintaan_uditch($ids)
    {
        $dt = new DateTime("now", new DateTimeZone('Asia/Jakarta'));
        $data = array(
            'marketing'     => $this->input->post('marketing'),
        ...
        $update = $this->db->where('ids', $ids)->update('tbl_permintaan_uditch', $data);	
    ...
    public function pdf_data_all_uditch($id)
    {
        $query = $this->db->query("SELECT * FROM tbl_data_penawaran where id='$id' and produk='Uditch' ");
        return $query->result_array();
    }

    public function pdf_data_permintaan_uditch($id)
    {
        $query = $this->db->query("SELECT penawaran.nama_proyek, penawaran.id, uditch.ids, uditch.nama_proyek, uditch.marketing, uditch.item,uditch.harga_satuan , uditch.jumlah, uditch.beban_gandar, uditch.keterangan , penawaran.include ,penawaran.ppn ,(uditch.jumlah * uditch.harga_satuan) as total_perkalian  FROM tbl_permintaan_uditch uditch JOIN tbl_data_penawaran penawaran on penawaran.nama_proyek=uditch.nama_proyek WHERE penawaran.id='$id' and penawaran.produk='Uditch'");
        return $query->result();
    }

    public function pdf_hitung_ppn($id)
    {
        $query = $this->db->query("SELECT penawaran.nama_proyek, penawaran.id, uditch.ids, uditch.nama_proyek, uditch.harga_satuan , uditch.jumlah, uditch.beban_gandar, penawaran.ppn ,sum((uditch.jumlah * uditch.harga_satuan)*0.11) as total_ppn  FROM tbl_permintaan_uditch uditch JOIN tbl_data_penawaran penawaran on penawaran.nama_proyek=uditch.nama_proyek WHERE penawaran.id='$id' and penawaran.produk='Uditch'");
        return $query->result_array();
    }
    ...
    {
        $query = $this->db->query("SELECT penawaran.nama_proyek, penawaran.id, uditch.ids, uditch.nama_proyek,uditch.harga_satuan , uditch.jumlah, uditch.beban_gandar , uditch.keterangan, penawaran.ppn ,sum(uditch.jumlah * uditch.harga_satuan) as total_jumlah  FROM tbl_permintaan_uditch uditch JOIN tbl_data_penawaran penawaran on penawaran.nama_proyek=uditch.nama_proyek WHERE penawaran.id='$id' and penawaran.produk='Uditch'");
        return $query->result_array();
    }

    public function pdf_total_jumlah_total_ppn($id)
    {
        $query = $this->db->query("SELECT penawaran.nama_proyek, penawaran.id, uditch.ids, uditch.nama_proyek, uditch.harga_satuan , uditch.jumlah , uditch.beban_gandar , uditch.keterangan, penawaran.ppn ,sum(uditch.jumlah * uditch.harga_satuan) as total_jumlah ,sum((uditch.jumlah * uditch.harga_satuan)*0.11) as total_ppn FROM tbl_permintaan_uditch uditch JOIN tbl_data_penawaran penawaran on penawaran.nama_proyek=uditch.nama_proyek WHERE penawaran.id='$id' and penawaran.produk='Uditch'");
        return $query->result_array();
    }

    public function pdf_get_bulan_uditch($id)
    {
        $query = $this->db->query("SELECT MONTH(tanggal) as tanggal from tbl_data_penawaran where id='$id' and produk='Uditch'");
        return $query->result_array();
    }

    public function pdf_get_tahun_uditch($id)
    {
        $query = $this->db->query("SELECT YEAR(tanggal) as tanggal from tbl_data_penawaran where id='$id' and produk='Uditch'");
        return $query->result_array();
    }

    public function pdf_get_tanggal_uditch($id)
    {
        $query = $this->db->query("SELECT DAY(tanggal) as tanggal from tbl_data_penawaran where id='$id' and produk='Uditch'");
        return $query->result_array();
    }
}