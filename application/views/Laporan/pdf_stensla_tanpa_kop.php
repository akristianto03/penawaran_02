...
<body>
  ...
  <p align="center">
    PENAWARAN HARGA STENSLA
    <?php foreach ($pdf_data_all_stensla as $rows) : ?><?= $rows['include'] ?><?php endforeach; ?>
    <br>
    <b>No. <?php foreach ($pdf_data_all_stensla as $row) : ?><?= $row['no_surat'] ?><?php endforeach; ?>/
      <?php foreach ($pdf_data_all_stensla as $row) : ?>
        ...
        <?php } ?>
        <?php endforeach; ?>/
        <?php foreach ($pdf_data_bulan_stensla as $row) : ?>
          ...
          <?php } ?>
          <?php endforeach; ?>/
          <?php foreach ($pdf_data_tahun_stensla as $row) : ?><?= $row['tanggal'] ?><?php endforeach; ?>
  </p>
  <p align="right" style="font-weight:bold">Surabaya,
    <?php foreach ($pdf_data_tanggal_stensla as $row) : ?>
      <?= $row['tanggal'] ?>
    <?php endforeach; ?>
    <?php foreach ($pdf_data_bulan_stensla as $row) : ?>
      <?php if ($row['tanggal'] == '1') { ?>
  ...
  <table class="tabless">
    <thead>
      <tr>
        ...
        <th class="ths" style="text-align:left !important">Armada</th>
    ...
    <tbody>
      <?php
      $no = 1;
      foreach ($pdf_data_permintaan_stensla as $row) :
      ?>
        <tr>
          <td class="tds"><?= $no++ ?></td>
          <td class="tds"><?= $row->item ?></td>
          <td class="tds"><?= $row->armada ?></td>
  ...
  <ol>
    <li>Harga
      <ol>
        <?php foreach ($pdf_data_all_stensla as $rows) : ?>
          <?php if ($rows['include'] == 'Loco') { ?>
            <li>Pengambilan di lokasi plant PT Bumindo Sakti di Driyorejo, Gresik</li>
          <?php } else { ?>
            <li>Harga SUDAH TERMASUK PENGIRIMAN SAMPAI KE LOKASI </li>
          <?php } ?>
        <?php endforeach; ?>
        <!-- <li>Bila muatan tidak mencapai jumlah penuh (7 kubik untuk Colt Diesel / 21 Kubik untuk Tronton) maka akan dikenakan biaya tambahan</li> -->
