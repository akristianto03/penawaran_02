...
<body>
  ...
  <p align="center">
    ...
    <br>
    <b>No. <?php foreach ($pdf_data_all_readymix as $row) : ?><?= $row['no_surat'] ?><?php endforeach; ?>/
      <?php foreach ($pdf_data_all_readymix as $row) : ?>
        ...
        <?php } ?>
        <?php endforeach; ?>/
        <?php foreach ($pdf_data_bulan_readymix as $row) : ?>
          ...
          <?php } ?>
          <?php endforeach; ?>/
          <?php foreach ($pdf_data_tahun_readymix as $row) : ?><?= $row['tanggal'] ?><?php endforeach; ?>
  </p>
  <p align="right" style="font-weight:bold">Surabaya,
    <?php foreach ($pdf_data_tanggal_readymix as $row) : ?>
      <?= $row['tanggal'] ?>
    <?php endforeach; ?>
    <?php foreach ($pdf_data_bulan_readymix as $row) : ?>
      <?php if ($row['tanggal'] == '1') { ?>
  ...
  <table class="tabless">
    <thead>
      <tr>
        ...
        <th class="ths" style="text-align:left !important">Slump</th>
        <th class="ths" style="text-align:left !important">Umur Beton</th>
        <th class="ths" style="text-align:left !important">Tipe</th>
    ...
    <tbody>
      <?php
      $no = 1;
      foreach ($pdf_data_permintaan_readymix as $row) :
      ?>
        <tr>
          <td class="tds"><?= $no++ ?></td>
          <td class="tds"><?= $row->item ?></td>
          <td class="tds"><?= $row->slump ?></td>
          <td class="tds"><?= $row->umur_beton ?></td>
          <td class="tds"><?= $row->tipe ?></td>
      ...
      ?>
        <tr>
          <td class="tds" colspan="8">Total</td>
      ...
      <?php foreach ($pdf_hitung_ppn as $rows) : ?>
        <?php if ($rows['ppn'] == 'Ya') { ?>
          ...
          <?php foreach ($pdf_hitung_ppn as $rows) : ?>
            <tr>
              <td class="tds" colspan="8">Total PPN</td>
              <?php if ($rows['ppn'] == 'Ya') { ?>
          ...
          <?php foreach ($pdf_total_jumlah_total_ppn as $rows) : ?>
            <tr>
              <td class="tds" colspan="8">Total BAYAR</td>
              <?php if ($rows['ppn'] == 'Ya') { ?>
  ...
  <?php foreach ($pdf_data_all_readymix as $row) : ?>
    <ol>
      <li>Spesifikasi teknis produk mutu beton diatas sesuai dengan peraturan Standart Nasional Indonesia (SNI 2847:2013).</li>
      <li>Pembayaran berlaku : Tunai Sebelum Pengecoran </li>
      <li>Akan dikenakan bunga 0.04 % perhari, selambat2nya 15 hari setelah jatuh tempo </li>
      <li>Penawaran harga beton ini berlaku sampai : 30 hari, atau jika tidak ada kenaikan harga bahan baku, harga bahan bakar minyak (BBM) dan kebijakan pemerintah di bidang moneter</li>
      <li>Purchase order kurang dari 4m3 dikenakan biaya Rp.350.000,-/tm</li>
      <li>Pemakaian batu maksimum 25 mm, apabila ada perubahan dari ukuran batu tersebut (batu 1-1) dikenakan tambahan biaya @ Rp. 25.000,- per m³, penambahan slump per 2 cm dikenakan tambahan biaya @ Rp. 25.000,- per m³ dan permintaan additive superplastisizer dikenakan tambahan biaya@ Rp. 25.000,- per liter</li>
      <li>Pengecoran lebih dari 4 jam dikenakan biaya overtime @Rp 200.000 / jam</li>
      <li>Biaya pengetesan di luar laboratorium PT. Bumindo Sakti menjadi tanggung jawab pembeli dan pengetesanya disaksikan bersama</li>
      <li>Pengaduan keluhan non teknis (volume beton, pengiriman beton dan lain-lain) akan kami layani maximum dalam waktu 2 x 24 jam dengan syarat tertulis, sedangkan keluhan teknis akan kami layani maximum 28 hari dengan syarat tertulis juga, terhitung dari tanggal pengecoran. Keluhan setelah tenggang waktu tersebut tidak akan kami layani, termasuk kerugian yang ditimbulkannya</li>
      <li>Penuangan beton dalam truck mixer waktunya lebih dari 4 jam pihak penjual tidak bertanggung jawab terhadap mutu beton tersebut</li>
      <li>Pihak pembeli bertanggung jawab atas tersedianya jalan yang layak dan dapat dilalui truck pengangkut (truck mixer) serta retribusi atau pungutan yang terjadi menuju ke lokasi proyek</li>
      <li>Pengiriman beton ke lokasi proyek dapat dilaksanakan setelah kami menerima copy penawaran harga yang telah disetujui pihak pembeli. </li>
    </ol>
  <?php endforeach; ?>
      ...
      <?php foreach ($pdf_data_all_readymix as $row) : ?>
        ...
        <?php } elseif ($row['marketing'] == 'candra5758') { ?>
          <tr>
            <td>
              <div style="position:relative;top:25px;left:-30px"><img src="./assets/ttd/stempel.png" style="width:120px"></div>
              <div style="position:absolute;top:10px"><img src="./assets/ttd/candra.png" style="width:140px"></div>
            </td>
            <td align="right"></td>
          </tr>
        <?php } elseif ($row['marketing'] == 'Nico') { ?>
          <tr>
            <td>
              <div style="position:relative;top:25px;left:-30px"><img src="./assets/ttd/stempel.png" style="width:120px"></div>
              <div style="position:absolute;top:10px"><img src="./assets/ttd/nico.png" style="width:140px"></div>
            </td>
            <td align="right"></td>
          </tr>
        <?php } elseif ($row['marketing'] == 'Agung') { ?>
          <tr>
            <td>
              <div style="position:relative;top:25px;left:-30px"><img src="./assets/ttd/stempel.png" style="width:120px"></div>
              <div style="position:absolute;top:10px"><img src="./assets/ttd/Agung.png" style="width:140px"></div>
            </td>
            <td align="right"></td>
          </tr>
        <?php } elseif ($row['marketing'] == 'Mohamad') { ?>
          <tr>
            <td>
              <div style="position:relative;top:25px;left:-30px"><img src="./assets/ttd/stempel.png" style="width:120px"></div>
              <div style="position:absolute;top:10px"><img src="./assets/ttd/Mohamad.png" style="width:140px"></div>
            </td>
            <td align="right"></td>
          </tr>
        <?php } ?>
