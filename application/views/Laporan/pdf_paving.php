...
<body>
  ...
  <p align="center">
    PENAWARAN HARGA PAVING
    <br>
    <b>No. <?php foreach ($pdf_data_all_paving as $row) : ?><?= $row['no_surat'] ?><?php endforeach; ?>/
      <?php foreach ($pdf_data_all_paving as $row) : ?>
        ...
        <?php } ?>
        <?php endforeach; ?>/
        <?php foreach ($pdf_data_bulan_paving as $row) : ?>
          ...
          <?php } ?>
          <?php endforeach; ?>/
          <?php foreach ($pdf_data_tahun_paving as $row) : ?><?= $row['tanggal'] ?><?php endforeach; ?>
  </p>
  <p align="right" style="font-weight:bold">Surabaya,
    <?php foreach ($pdf_data_tanggal_paving as $row) : ?>
      <?= $row['tanggal'] ?>
    <?php endforeach; ?>
    <?php foreach ($pdf_data_bulan_paving as $row) : ?>
      <?php if ($row['tanggal'] == '1') { ?>
    ...
    <?php if ($row['include'] == "Jasa") { ?>
      ...
      <table class="tabless">
        ...
        <tbody>
          <?php
          $no = 1;
          foreach ($pdf_data_permintaan_paving as $row) :
          ?>
            <tr>
              <td class="tds"><?= $no++ ?></td>
      ...
      <table class="tabless">
        <thead>
          <tr>
            <th class="ths" style="text-align:left !important">No.</th>
            <th class="ths" style="text-align:left !important">Item</th>
            <th class="ths" style="text-align:left !important">Mutu Beton</th>
            <th class="ths" style="text-align:left !important">Warna</th>
            <th class="ths" style="text-align:left !important">Satuan</th>
            <th class="ths" style="text-align:left !important">Jumlah</th>
            <th class="ths" style="text-align:left !important">Harga Satuan</th>
            <th class="ths" style="text-align:left !important">Total Harga</th>
        ...
        <tbody>
          ...
          ?>
            <tr>
              <td class="tds"><?= $no++ ?></td>
              <td class="tds"><?= $row->item ?></td>
              <td class="tds"><?= $row->mutu_beton ?></td>
              <td class="tds"><?= $row->warna ?></td>
              <td class="tds"><?= $row->satuan ?></td>
              <td class="tds"><?= $row->jumlah ?></td>
              <td class="tds">Rp. <?= number_format($row->harga_satuan, 2, ',', '.') ?></td>
              <td class="tds">Rp. <?= number_format($row->total_perkalian, 2, ',', '.') ?></td>
  ...
  <?php foreach ($pdf_data_all_paving as $row) : ?>
    <?php if ($row['include'] == "Jasa") { ?>
      <strong>A. SYARAT & KETENTUAN</strong>
      <ol>
        <?php foreach ($pdf_data_all_paving as $row) : ?>
          <?php if ($row['ppn'] == "Ya") { ?>
            <!--<li></li>-->
      ...
      <ol>
        <li>Jasa pemasangan termasuk = Perataan lahan, Gelar sand bedding, Pasang Paving, Tabur Filler, Pemadatan, Pembersihan</li>
        ...
        <li>Segala keamanan di lokasi proyek adalah tanggung jawab pemberi tugas, bila terjadi kerusakan/kerusuhan di lokasi proyek maka pemberi tugas wajib mengganti segala kerugian yang telah ditimbulkan</li>
      </ol>
    <?php } else { ?>
      <strong>A. Tentang Harga dan Biaya Lain-Lain</strong>
      <ol>
        <li>Harga sudah termasuk pengiriman dan penurunan di lokasi menggunakan tenaga kerja dari PT BUMINDO SAKTI (Manual)</li>
        <?php foreach ($pdf_data_all_paving as $row) : ?>
          <?php if ($row['ppn'] == "Ya") { ?>
            <!--<li></li>-->
          <?php } else { ?>
        ...
        <?php endforeach; ?>
        <li>Harga sewaktu-waktu dapat berubah, apabila terjadi force majeure, perubahan moneter, BBM, dari pemerintah</li>
        <li>Biaya pengetesan di laboratorium independent menjadi tanggungan pembeli</li>
        <li>Order akan diproses setelah penawaran ini di tanda dan dikirim ke pihak kami</li>
        <li>Membayar DP/termin pembayaran berarti menyetujui dan sudah membaca semua ketentuan di atas meskipun tidak menandatangani penawaran ini</li>
        <li>Penawaran ini berlaku 2 minggu dari tanggal diterbitkan</li>
        <li>Harga belum termasuk packing menggunakan pallet kayu untuk ekspedisi luar pulau (bila ada request)</li>
        <li>Minimal pengiriman adalah 4 Pallet, bila dibawah 4 Pallet maka akan di kenakan biaya ONGKOS KIRIM Rp. 550.000/rit untuk daerah Surabaya, Sidoarjo, Mojokerto, Gresik (40 Km dari plant PT BUMINDO SAKTI), bila lokasi diluar radius 40 Km maka ongkos kirim akan disesuaikan</li>
      </ol>

      <strong>B. Tentang Barang</strong>
      <ol>
        <li>Toleransi ukuran paving adalah 8% dari ukuran standard (SNI 03-0691-1996)</li>
        <li>Kerusakan paving yang diakibatkan oleh kurangnya pemadatan lahan bukan menjadi tanggung jawab PT BUMINDO SAKTI</li>
        <li>Proses Produksi maksimal 28 hari kerja bila barang tidak ready, terhitung dari DP masuk</li>
        <li>Barang yang dipesan tidak dapat dibatalkan atau dikurangi</li>
        <li>Komplain terhadap barang maksimal 2x24 jam terhitung dari barang telah diterima oleh penanggung jawab lapangan pembeli, lewat dari waktu tersebut, PT BUMINDO SAKTI tidak menerima komplain/retur barang</li>
      </ol>

      <strong>C. Tentang Pengiriman</strong>
      <ol>
        <li>Kerusakan pada akses yang ditimbulkan armada pengiriman menjadi tanggung jawab pembeli</li>
        <li>Segala macam biaya perijinan masuk, keamanan, portal, LSM, iuran warga merupakan tanggung jawab pembeli</li>
      </ol>
  ...
  <strong>D. Tentang Pembayaran</strong>
  <ol>
    <?php foreach ($pdf_data_all_paving as $row) : ?>
      <?php if ($row['ppn'] == 'Ya') { ?>
      ...
      <?php } else { ?>
        ...
        <li>30% dibayar pada saat penandatangan kontrak (DP)</li>
        <li>70% dibayar pada saat barang siap dikirim ke lokasi</li>
