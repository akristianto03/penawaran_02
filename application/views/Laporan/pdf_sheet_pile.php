...
<body>
  <?php foreach ($pdf_data_all_sheet_pile as $row) : ?>
    ...
    <?php } elseif ($row['include'] == 'Jasa') { ?>
      <div style="position:absolute;top:-64px !important;left:-60px !important;"><img src="./assets/kop_surat/srikandi_header.png" style="height:160px;width:800px"></div>
  ...
  <div style="position:relative">
    <p align="center">
      PENAWARAN HARGA SHEET PILE
      <br>
      <b>No. <?php foreach ($pdf_data_all_sheet_pile as $row) : ?><?= $row['no_surat'] ?><?php endforeach; ?>/
        <?php foreach ($pdf_data_all_sheet_pile as $row) : ?>
          ...
          <?php } ?>
          <?php endforeach; ?>/
          <?php foreach ($pdf_data_bulan_sheet_pile as $row) : ?>
            ...
            <?php } ?>
            <?php endforeach; ?>/
            <?php foreach ($pdf_data_tahun_sheet_pile as $row) : ?><?= $row['tanggal'] ?><?php endforeach; ?>
    </p>
    <p align="right" style="font-weight:bold">Surabaya,
      <?php foreach ($pdf_data_tanggal_sheet_pile as $row) : ?>
        <?= $row['tanggal'] ?>
      <?php endforeach; ?>
      <?php foreach ($pdf_data_bulan_sheet_pile as $row) : ?>
        <?php if ($row['tanggal'] == '1') { ?>
    ...
    <table class="tabless">
      ...
      <tbody>
        <?php
        $no = 1;
        foreach ($pdf_data_permintaan_sheet_pile as $row) :
        ?>
          <tr>
            <td class="tds"><?= $no++ ?></td>
    ...
    <ol>
      <li>Harga
        <ol>
          <li>Jangka waktu penawaran berlaku 14 (empat belas hari) hari dari tanggal terbit surat penawaran harga</li>
          <li>Harga sewaktu-waktu dapat berubah, apabila terjadi perubahan moneter, BBM, dan HPS Semen dari Pemerintah</li>
        </ol>
      </li>
      <li>Pengiriman
        <ol>
          <li>Pengiriman menggunakan armada tronton (memakai colt diesel jika sudah ada kesepakatan di awal)</li>
          <li>Akses menuju ke lokasi proyek harus disiapkan dengan baik agar bisa dilewati tronton (tidak ambles, tidak ada gangguan kabel,dll)</li>
          <li>Schedule pengiriman sesuai dengan kesepakatan kedua belah pihak</li>
          <li>Harga belum termasuk biaya langsir, jika mobil tidak bisa masuk ke lokasi proyek</li>
        </ol>
      </li>
      <li>Biaya Tambahan
        <ol>
          <li>Harga diatas belum termasuk biaya koordinasi dengan lingkungan proyek</li>
          <li>Harga diatas belum termasuk biaya repair jika ada kelalaian (akibat kelalaian Handling di lapangan)</li>
          <li>Biaya tambahan dikenakan jika barang yang sudah terkirim di lokasi proyek tapi tidak diturunkan (tidak ada penerima) dan kembali dibawa ke Pabrik</li>
        </ol>
      </li>
      <li>Syarat & Metode Pembayaran
        <ol>
          <?php foreach ($pdf_data_all_sheet_pile as $row) : ?>
            <?php if ($row['ppn'] == 'Ya') { ?>
              <b>Harga Sudah Termasuk PPN</b <p>Nomor Rekening : 1840628899</p>
              <p>Bank : BCA</p>
