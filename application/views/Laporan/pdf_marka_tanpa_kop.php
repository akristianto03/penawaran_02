...
<body>
  ...
  <p align="center">
    PENAWARAN HARGA MARKA JALAN
    <br>
    <b>No. <?php foreach ($pdf_data_all_marka as $row) : ?><?= $row['no_surat'] ?><?php endforeach; ?>/
      <?php foreach ($pdf_data_all_marka as $row) : ?>
        ...
        <?php } ?>
        <?php endforeach; ?>/
        <?php foreach ($pdf_data_bulan_marka as $row) : ?>
          ...
          <?php } ?>
          <?php endforeach; ?>/
          <?php foreach ($pdf_data_tahun_marka as $row) : ?><?= $row['tanggal'] ?><?php endforeach; ?>
  </p>
  <p align="right" style="font-weight:bold">Surabaya,
    <?php foreach ($pdf_data_tanggal_marka as $row) : ?>
      <?= $row['tanggal'] ?>
    <?php endforeach; ?>
    <?php foreach ($pdf_data_bulan_marka as $row) : ?>
      <?php if ($row['tanggal'] == '1') { ?>
  ...
  <table class="tabless">
    <thead>
      <tr>
        ...
        <th class="ths" style="text-align:left !important">Standard</th>
    ...
    <tbody>
      <?php
      $no = 1;
      foreach ($pdf_data_permintaan_marka as $row) :
      ?>
        <tr>
          <td class="tds"><?= $no++ ?></td>
          <td class="tds"><?= $row->item ?></td>
          <td class="tds"><?= $row->standard ?></td>
      ...
      ?>
        <tr>
          <td class="tds" colspan="6">Total PPN</td>
          <?php if ($rows['ppn'] == 'Ya') { ?>
      ...
      ?>
        <tr>
          <td class="tds" colspan="6">Total BAYAR</td>
          <?php if ($rows['ppn'] == 'Ya') { ?>
  ...
  <strong>Lebar Marka</strong>
  <ol>
    <li>Zebra Cross : Lebar 30 cm Panjang 2.5 meter</li>
    <li>Solid: Lebar 12 cm</li>
    <li>Putus-Putus : Lebar 12 cm, Panjang 3 meter dengan jarak interval 5 meter tiap marka</li>
  </ol>
  ...
  <ol>
    <li>Harga
      <ol>
        <li>Minimal Volume pekerjaan adalah 60 M2</li>
        <li>Harga SUDAH TERMASUK biaya material dan pekerjaan</li>
        <li>Harga tersebut berlaku untuk pekerjaan diatas permukaan Aspal. Bila pekerjaan dilakukan di permukaan beton maka akan ada biaya tambahan sebesar Rp. 5.000/M2</li>
        <li>Biaya pengetesan Reflektivitas menjadi tanggungan pemberi tugas</li>
        <li>Harga diatas sudah termasuk jasa traffic management</li>
        <li>Harga belum termasuk pekerjaan tambahan yang timbul di lapangan diluar lingkup pekerjaan (pembersihan lahan akibat hujan, lumpur)</li>
      </ol>
    </li>
    <li>Keterangan
      <ol>
        <li>Jangka waktu penawaran berlaku 2 Minggu sejak diterbitkannya penawaran</li>
        <li>Kerusakan pada akses yang ditimbulkan armada pengiriman(truck engkel) menjadi tanggung jawab customer</li>
