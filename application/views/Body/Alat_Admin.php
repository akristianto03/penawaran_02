...
<div id="content">
        ...
        <div class="card shadow mb-4">
            ...
            <div class="card-body">
                <div class="table-responsive">
                    <table style="font-size:10px" class="table table-bordered" id="alat" width="10%" cellspacing="0">
                        <thead>
                            <tr>
                                <th>No</th>
                                <th>Nama Instansi</th>
                        ...
                        <tbody>
                            <?php 
                            $no = 1;
                            foreach($data_alat_admin as $row ):?>
                            <tr>
                                <td><?= $no++ ?></td>
                                ...
                                <?php if($_SESSION['username'] == 'Rini') {?>
                                    <td><a class="btn btn-warning btn-sm" href="<?= base_url()?>Laporan/cetak_alat/<?= $row->id?>">Cek Dokumen Pdf</a></td>
                                <?php } else {?>

                                <?php if($row->image != NULL) {?>
                                    <?php if($row->goals != NULL) {?>
                                            <td><a href="<?= base_url()?>Alat/ungoals/<?= $row->id?>" class="btn btn-danger btn-sm">Undo Goals</a></td>
                                        <?php } else {?>
                                            <td><a href="<?= base_url()?>Alat/goals/<?= $row->id?>" class="btn btn-success btn-sm">Goals</a></td>
                                    <?php }?>
                                <?php } else {?>
                                ...
                                <?php if($row->status > '0') {?>
                                        <td><a href="<?= base_url()?>Alat/unapprove/<?= $row->id?>" class="btn btn-danger btn-sm">Unauthorized</a></td>
                                <?php } else {?>
                                        <td><a href="<?= base_url()?>Alat/approve/<?= $row->id?>" class="btn btn-success btn-sm">Authorization</a></td>
                                <?php }?>
                                    <td><a href="<?= base_url()?>Alat/detail_alat/<?= $row->id?>" class="btn btn-info btn-sm">Berikan Harga</a></td>
    ...
    <?php foreach($data_alat_admin as $pp) : ?>
    <div class="modal fade" id="edit<?= $pp->id?>" tabindex="-1" role="dialog" aria-labelledby="myExtraLargeModalLabel" style="display: none;" aria-hidden="true">
        <div class="modal-dialog modal-xl" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">Catatan Penawaran Alat</h5>
                    <button class="close" type="button" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">×</span></button>
                </div>
                <div class="modal-body">
